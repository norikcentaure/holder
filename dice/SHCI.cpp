  ...
  for (int d = 0; d < HFoccupied.size(); d++) {
    ...
    */
    ...
    if (schd.DoFourRDM) {
      ...
      //TEST//pout << norbs << endl; pout<<std::flush;
      //TEST//for (int i=26; i<32; i+=2){
      //TEST//  norbs = i;
      //TEST//  pout << "(setZero try..) " << norbs/2 << " " << norbs/2*norbs/2*norbs/2*norbs/2 << endl;pout << std::flush;
      //TEST//  s4RDM.setZero(norbs*norbs*norbs*norbs/16,norbs*norbs*norbs*norbs/16);
      //TEST//  pout << "(setZero is ok)" << endl;pout << std::flush;
      //TEST//}
