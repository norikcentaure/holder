  ...
  */
  ...
  if (commrank == 0) {
    ...
    if (schd.DoSpinRDM) {
      ...
      if (true)
      {
      pout << "  (save bin spin file)" << endl;
      pout << std::flush;
      char file[5000];
      sprintf(file, "%s/spin2RDM.%d.%d.bin", schd.prefix[0].c_str(), root, root);
      std::ofstream ofs(file, std::ios::binary);
      boost::archive::binary_oarchive save(ofs);
      save << twoRDM;
      }
      ...
      if (true)
      {
        ...
        pout << std::flush;
        char file[5000];
        sprintf(file, "%s/spin2RDM.%d.%d.txt", schd.prefix[0].c_str(), root, root);
        std::ofstream ofs(file, std::ios::out);
        ofs << 2*nSpatOrbs << endl;
        for (int n1 = 0; n1 < 2*nSpatOrbs; n1++)
          for (int n2 = 0; n2 < n1; n2++)
            for (int n3 = 0; n3 < 2*nSpatOrbs; n3++)
...
void SHCIrdm::save3RDM(schedule &schd, MatrixXx &threeRDM, MatrixXx &s3RDM,
  ...
  int n = norbs / 2; // pow(s3RDM.rows(),1/3.0);
  int n2 = n * n;
  ...
  if (commrank == 0) {
    ...
    if (false)
    {
      pout << "  (save npy file)" << endl;
      pout << std::flush;

      char desc[1000]="{'descr': '<f8', 'fortran_order': True, 'shape': (";
      char buffer[3];
      sprintf(buffer, "%d", n);
      strcat(desc, buffer);
      for(int i=1;i<6;i++){
      ...
      FILE *fp;
      char file[5000];
      char header[1000]="\x93NUMPY";
      int x;
      sprintf(file, "%s/E3.npy",schd.prefix[0].c_str());
      fp=fopen(file,"w");
      fwrite(&header[0],sizeof(char),strlen(header),fp);
      x=1;            fwrite(&x,sizeof(int8_t),1,fp);
      x=0;            fwrite(&x,sizeof(int8_t),1,fp);
      x=strlen(desc); fwrite(&x,sizeof(int16_t),1,fp);
      fwrite(&desc[0]  ,sizeof(char),strlen(desc),  fp);
      ...
      for (int c2 = 0; c2 < n; c2++)
        ...
        for (int d0 = 0; d0 < n; d0++)
          fwrite(&s3RDM(n2*c0 + n*c1 + c2,
                        n2*d0 + n*d1 + d2),sizeof(s3RDM(0)),1,fp);
      fclose(fp);
    }
...
void SHCIrdm::save4RDM(schedule &schd, MatrixXx &fourRDM, MatrixXx &s4RDM,
  ...
  if (commrank == 0) {
    ...
    if (false)
    {
      ...
      char header[1000]="\x93NUMPY";
      int x;
      sprintf(file, "%s/E4.npy",schd.prefix[0].c_str());
      fp=fopen(file,"w");
      fwrite(&header[0],sizeof(char),strlen(header),fp);
      x=1;            fwrite(&x,sizeof(int8_t),1,fp);
      x=0;            fwrite(&x,sizeof(int8_t),1,fp);
      ...
      for (int c3 = 0; c3 < n; c3++)
        ...
        for (int d1 = 0; d1 < n; d1++)
        for (int d0 = 0; d0 < n; d0++)
          fwrite(&s4RDM(n3*c0 + n2*c1 + n*c2 + c3,
                        n3*d0 + n2*d1 + n*d2 + d3),sizeof(s4RDM(0)),1,fp);
      fclose(fp);
    }
