...
class SHCI(pyscf.lib.StreamObject):
    ...
    def make_rdm3(self, state, norb, nelec, dt=numpy.dtype('Float64'),
                  filetype = "binary", bypass=False, link_index=None,
    ...
    def make_rdm4(self, state, norb, nelec, dt=numpy.dtype('Float64'), filetype = "binary", bypass=False, link_index=None, **kwargs):
        import os
    ...
    def kernel(self, h1e, eri, norb, nelec, fciRestart=None, ecore=0, **kwargs):
        ...
        #HERE#print('BM_solvers_B',self.__class__)
        #HERE#print('BM_solvers_B',self.__class__.__name__)
        #HERE#print('BM_solvers_B',self)
        executeSHCI(self)
        if self.verbose >= logger.DEBUG1:
            outFile = os.path.join(self.runtimeDir, self.outputFile)
            logger.debug1(self, open(outFile).read())
