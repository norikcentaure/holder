...
class DMRGCI(lib.StreamObject):
    ...
    def dump_flags(self, verbose=None):
        ...
        log.info('scheduleSweeps         = %s', '['+','.join(['{:>5}' for item in self.scheduleSweeps]).format(*self.scheduleSweeps)+']')
        log.info('scheduleMaxMs          = %s', '['+','.join(['{:>5}' for item in self.scheduleMaxMs]).format(*self.scheduleMaxMs)+']')
        log.info('scheduleTols           = %s', '['+','.join(['{:>5}' for item in self.scheduleTols]).format(*self.scheduleTols)+']')
        log.info('scheduleNoises         = %s', '['+','.join(['{:>5}' for item in self.scheduleNoises]).format(*self.scheduleNoises)+']')
