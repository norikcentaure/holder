...
def decomp_4rdm_to_3rdm_sf(d4, d1, d2, d2_hom, d2_het, d3):
  ...
  for p in range(4):
    ...
    for q in range(4):
      ...
      coeff = sign * (0.5)**n_perm
      d1_copy_0 = d1.copy()
      d1_copy_0.indices = [d4.indices[i] for i in (ti[:1] + bj[:1])]
      d3_copy_0 = d3.copy()
      d3_copy_0.indices = [d4.indices[i] for i in (ti[1:] + bj[1:])]
      ...
      for r in range(4):
        for s in range(r+1,4):
          ...
          if n_perm < 2:
            coeff = sign * (0.5)**n_perm
            d2_copy_0 = d2.copy()
            d2_copy_0.indices = [d4.indices[i] for i in (ti[:2] + bj[:2])]
            d2_copy_1 = d2.copy()
            d2_copy_1.indices = [d4.indices[i] for i in (ti[2:] + bj[2:])]
            decomp.append(term(coeff, [], [d2_copy_0, d2_copy_1]))
          elif n_perm == 2:
            coeff = sign * 0.5
            d2_hom_copy_0 = d2_hom.copy()
