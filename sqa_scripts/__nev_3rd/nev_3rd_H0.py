...
for this_class in class_list+['other']:
  ...
  for h_term in H:
    ...
    # and we exclude known patterns
    elif pattern(h_term) in pattern_of.values():
      continue

    if  code(indices_of_ExOp(h_term))!='aa'\
    and code(indices_of_ExOp(h_term))!='cc'\
    and code(indices_of_ExOp(h_term))!='vv':
      continue

    done.append(H.index(h_term))
    ...
    for left_term in psi1_left:
      for right_term in psi1_right:
        ...
        if is_non_zero([left_term,h_term,right_term]):
          ...
          result_for_class=simplify_all(result_for_class,[deltaC, deltaA, deltaV])
...
print '//',len(done),len(set(done)),set(done)
...
tensors    = pt3_tensors(result,'NEVPT')
nbrTensors = write_tensors('NEVPT3', tensors)
nbrEqs     = write_code   ('NEVPT3', tensors, result)
print_GetMethodInfo('NEVPT','3',nbrTensors,nbrEqs,0)
