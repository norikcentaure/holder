...
# Define Omu
list1=gimme('ccaa')
list2=[list1[i] for i in [2,0]]
list3=[list1[i] for i in [3,1]]
pvec  = sqa.tensor('p', list1, [])#symm(list1))
Omu   = sqa.term(1.0, [''],  [pvec,  sqa.sfExOp(list2), sqa.sfExOp(list3)])

# Define Onu
list1=gimme('ccaa')
list2=[list1[i] for i in [1,3]]
list3=[list1[i] for i in [0,2]]
...
Class = 'CCAA'
tensors    = common_tensors(result,'ccaa',pttype='MRLCC',df=True)
